...
//  IdeaNote
//
//  Created by superfly on 2023/6/3.
//
...
struct NewNoteView: View {
    ...
    var body: some View {
        NavigationView {
            ...
            .navigationBarTitle(viewModel.isAdd ? "新建笔记" : "编辑笔记",
                                    displayMode: .inline)
            .navigationBarItems(leading: closeBtnView(), trailing: saveBtnView())
            .toast(present: $viewModel.showToast, message: $viewModel.showToastMessage, aliment: .center)
        }
    }
        ...
        Button(action:{
            ...
            if(viewModel.isAdd) {
               // 判断标题是否为空
                ...
                // 判断内容是否为空
                if viewModel.isTextEmpty(text: viewModel.content) {
                    viewModel.showToast = true
    ...
    func titleView() -> some View {
        TextField("请输入编辑", text: viewModel.isAdd ? $viewModel.title : $noteModel.title)
        .padding()
    }
